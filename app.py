...
engine = create_engine('postgresql://fncalderong:Fncalderon91G@strategy.c1kvaa1zefdl.us-east-2.rds.amazonaws.com/strategy')
...
def filter_df(d_frame, exchange, margin, start_date, end_date):
    df_aux = d_frame[(d_frame['Exchange'] == exchange) & (d_frame['Margin'] == margin) & (d_frame['Entry time'] >= start_date) & (d_frame['Entry time'] <= end_date)]
    return df_aux
...
# df['yymm'] = df['Entry time'].dt.strftime('%Y-%m')
#
#
# import plotly.graph_objects as go
#
                            ...
                            html.Div(
                                ...
                                children=[
                                    ...
                                    dcc.RadioItems(
                                        id="leverage-select",
                                        options=[
                                            {'label': int(label), 'value': int(label)} for label in df['Margin'].unique()
                                        ],
                                        value=1,
                                        labelStyle={'display': 'inline-block'}
                                    ),
...
def exchangechange(exchange):
    df_aux = df[df['Exchange'] == exchange]['Entry time']
    new_star_date = df_aux.min().date()
    new_end_date = df_aux.max().date()
    return [new_star_date, new_end_date]
...
def update_bar(exchange, leverage, start_date, end_date):
    ...
    data = []
    data.append(go.Bar(
        x=dff_Short['Entry time'], y=dff_Short['Pnl (incl fees)'], name='Short'
    ))
    data.append(go.Bar(
        x=dff_Long['Entry time'], y=dff_Long['Pnl (incl fees)'], name='Long'
    ))
    return {
            'data': data, 'layout': {'barmode': 'group','title':'PNL'}
        }
...
def update_line(exchange, leverage, start_date, end_date):
    dff = filter_df(df, exchange, leverage, start_date, end_date)
    data1 = []
    data1.append(go.Scatter(
            x=dff['Entry time'], y=dff['BTC Price'], name="BTC"
        ))
    data2 = []
    data2.append(go.Scatter(
        x=dff['Entry time'], y=dff['Entry balance'], name="Balance"
    ))
    return {
        'data': data1,
        'layout': go.Layout(
